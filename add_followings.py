...
def write_correct_userinfo(sourse_userinfo, users):
    global current_following_count
    fn = 'correct_{}.txt'.format(datetime_str)
    ...
    with open(fn, 'a', encoding='utf-8') as f:
        ...
        while i < size_users and current_following_count < max_following_count:
            ...
            f.write(write_full_pattern.format(*userinfo))
            f.flush()
    ...
    try:
        ...
        while current_following_count < max_following_count and len(users_to_following) > 0:
            ...
            if api.searchUsername(users_to_following[0]):
                ...
                if processing_only_correct_users:
                    # processing only correct users sequentially
                    correct, not_correct = get_users_with_correct_fullname(users_union)
                    write_correct_userinfo('User №' + str(i + 1) + ' : ' + users_to_following[0], correct)
                else:
                    # processing all users sequentially
